...
def preprocess(image):
    ...
    enhance_mat = IDA.image_utils.image.enhance(gray_img, height, width)
    filtered_mat = IDA.image_utils.fft.LPF_blur(enhance_mat, 5)
...
if __name__ == "__main__":
    ...
    with tf.device('/GPU:0'):
        ...
        resized_img = IDA.image_utils.image.resize(original_img, 256, 256, method='bicubic', antialias=True)
        ...
        LPF_blur_mat = IDA.image_utils.fft.LPF_blur(preprocessed_img, k)
        thr_lpf_mat = IDA.image_utils.threshold.std_threshold(LPF_blur_mat, k=thr_k)
        ...
        reconstruct_mat = IDA.image_utils.reconstruction.dct_reconstruction(preprocessed_img, 1)
        thr_dct_mat = IDA.image_utils.threshold.std_threshold(reconstruct_mat, k=thr_k)
        ...
        polyfit_mat = IDA.image_utils.reconstruction.polynomial_reconstruction(preprocessed_img)
        thr_poly_mat = IDA.image_utils.threshold.std_threshold(polyfit_mat, k=thr_k)
        ...
        print(comp_1, comp_2)
        if comp_1 > comp_2:
            remove_outlier_bg = IDA.image_utils.image.remove_outlier(thr_dct_mat, 1)
            final_bg = IDA.image_utils.reconstruction.polynomial_reconstruction(thr_dct_mat)
            print("bg1")
        else:
